...
class TestParseNewsDate:
    def test_valid_date(self):
        from custom_components.vikis_vodovod_news.binary_sensor import parse_news_date
        from datetime import date

        assert parse_news_date("15.06.2026.") == date(2026, 6, 15)
...
class TestBinarySensor:
    def test_off_when_no_news(self):
        from custom_components.vikis_vodovod_news.binary_sensor import (
            VikisVodovodNewsWarning,
        )

        coord = MagicMock()
        coord.data = {"news": [], "last_id": 606, "last_scan": None}
        sensor = VikisVodovodNewsWarning(coord)
        assert sensor.is_on is False

    def test_on_when_future_date(self):
        from custom_components.vikis_vodovod_news.binary_sensor import (
            VikisVodovodNewsWarning,
        )
        ...
        coord.data = {
            "news": [
                {"id": 607, "date": future, "title": "Buduća najava", "description": "", "priority": False},
            ],
            "last_id": 607,
            "last_scan": None,
        }
    ...
    def test_on_when_today(self):
        ...
        coord.data = {
            "news": [
                {"id": 607, "date": today, "title": "Današnja najava", "description": "", "priority": True},
            ],
            "last_id": 607,
            "last_scan": None,
        }
    ...
    def test_off_when_past_date(self):
        from custom_components.vikis_vodovod_news.binary_sensor import (
            VikisVodovodNewsWarning,
        )
        from datetime import datetime, timedelta

        past = (datetime.now() - timedelta(days=7)).strftime("%d.%m.%Y")
        coord = MagicMock()
        coord.data = {
            "news": [
                {"id": 600, "date": past, "title": "Stara najava", "description": "", "priority": False},
            ],
            "last_id": 600,
            "last_scan": None,
        }
    ...
    def test_active_warnings_attribute(self):
        from custom_components.vikis_vodovod_news.binary_sensor import (
            VikisVodovodNewsWarning,
        )
        from datetime import datetime, timedelta
        ...
        coord = MagicMock()
        news = [
            {"id": 606, "date": past, "title": "Staro", "description": "", "priority": False},
            {"id": 607, "date": future, "title": "Novo", "description": "", "priority": True},
        ]
        coord.data = {"news": news, "last_id": 607, "last_scan": None}
        sensor = VikisVodovodNewsWarning(coord)
        attrs = sensor.extra_state_attributes
        assert len(attrs["active_warnings"]) == 1
        assert attrs["active_warnings"][0]["id"] == 607
        assert attrs["total_news"] == 2
    ...
    def test_device_class(self):
        ...
        sensor = VikisVodovodNewsWarning(MagicMock())
        assert sensor.device_class == "safety"
        assert sensor.unique_id == "vikis_vodovod_news_warning"